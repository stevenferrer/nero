...
func TestNewSQLiteRepoCStringId(t *testing.T) {
...
func (s *SQLiteRepository) Query(ctx context.Context, q *Queryer) ([]*internal.Example2, error) {
	tx, err := s.Tx(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *SQLiteRepository) QueryTx(ctx context.Context, tx nero.Tx, q *Queryer) ([]*internal.Example2, error) {
	txx, ok := tx.(*sql.Tx)
	if !ok {
		return nil, errors.New("expecting tx to be *sql.Tx")
	}
